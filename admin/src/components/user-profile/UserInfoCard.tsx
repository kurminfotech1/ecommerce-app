          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{adminData?.country || "N/A"}</p>
              </div>
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">State</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{adminData?.state || "N/A"}</p>
              </div>
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">City</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{adminData?.city || "N/A"}</p>
              </div>
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Postal Code</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{adminData?.postal_code || "N/A"}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={formik.handleSubmit} className="flex flex-col">
              ...
              <div className="mt-7">
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  <div className="col-span-2 lg:col-span-1">
                    ...
                    <Input name="first_name" type="text" value={formik.values.first_name} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                    {formik.touched.first_name && formik.errors.first_name && <p className="text-red-500 text-xs">{formik.errors.first_name}</p>}
                  </div>
                  <div className="col-span-2 lg:col-span-1">
                    <Label>Last Name</Label>
                    <Input name="last_name" type="text" value={formik.values.last_name} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                    {formik.touched.last_name && formik.errors.last_name && <p className="text-red-500 text-xs">{formik.errors.last_name}</p>}
                  </div>
                  <div className="col-span-2 lg:col-span-1">
                    <Label>Email Address</Label>
                    <Input name="email" type="text" value={formik.values.email} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                    {formik.touched.email && formik.errors.email && <p className="text-red-500 text-xs">{formik.errors.email}</p>}
                  </div>
                  <div className="col-span-2 lg:col-span-1">
                    <Label>Phone</Label>
                    <Input name="phone" type="text" value={formik.values.phone} onChange={formik.handleChange} onBlur={formik.handleBlur} />
                  </div>
                </div>
              </div>

              <div className="mt-7">
                <h5 className="mb-5 text-lg font-medium text-gray-800 dark:text-white/90 lg:mb-6">Address</h5>
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  <div>
                    <Label>Country</Label>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              <Button size="sm" type="submit" disabled={isLoading}>
                {isLoading ? (
                  <><Spinner size={20} color="white" className="me-2" /> Updating...</>
                ) : "Update"}
              </Button>
            </div>
          </form>
