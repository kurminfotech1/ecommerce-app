...
// ─────────────────────────────────────────────────────────────────
// Contact Confirmation Email  → sent to the USER
// Usage: sendContactConfirmationEmail(contactData)
// ─────────────────────────────────────────────────────────────────
export async function sendContactConfirmationEmail(data: ContactPayload) {
  const html = emailWrapper(`
    <h2 style="margin:0 0 8px;font-size:22px;color:#1a3d1a;font-weight:700;">
      We've Received Your Message 🌿
    </h2>
    <p style="margin:0 0 28px;font-size:15px;color:#4a5c4a;line-height:1.7;">
      Hi <strong style="color:#2d6b2d;">${data.fullName}</strong>, thank you for contacting
      <strong style="color:#2d6b2d;">Avshdh Organics</strong>! We're glad you reached out.
    </p>

    <!-- Confirmation Card -->
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f9f0;border-radius:12px;margin:0 0 28px;overflow:hidden;">
      <tr>
        <td style="padding:24px 28px;">
          <p style="margin:0 0 16px;font-size:15px;color:#2d3d2d;line-height:1.7;">
            Your inquiry has been successfully received. Our team will carefully review your message and get back to you at
            <strong style="color:#2d6b2d;">${data.email}</strong> within <strong>24–48 hours</strong>.
          </p>
          <p style="margin:0;font-size:15px;color:#2d3d2d;line-height:1.7;">
            In the meantime, feel free to explore our range of organic products on our website.
          </p>
        </td>
      </tr>
    </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
      <tr>
        <td style="padding:0 0 12px;">
          <p style="margin:0;font-size:11px;letter-spacing:2px;color:#6b8a6b;text-transform:uppercase;font-weight:600;">What Happens Next</p>
        </td>
      </tr>
      <tr>
        <td>
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="padding:10px 0;border-bottom:1px solid #e8f5e8;">
                <table cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="width:32px;font-size:18px;vertical-align:top;padding-top:2px;">✅</td>
                    <td style="font-size:14px;color:#2d3d2d;line-height:1.6;">
                      <strong style="color:#1a3d1a;">Message Received</strong> — We've got your inquiry safe and sound.
                    </td>
                  </tr>
                </table>
              </td>
              ...
              <td style="padding:10px 0;border-bottom:1px solid #e8f5e8;">
                <table cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="width:32px;font-size:18px;vertical-align:top;padding-top:2px;">🔍</td>
                    <td style="font-size:14px;color:#2d3d2d;line-height:1.6;">
                      <strong style="color:#1a3d1a;">Under Review</strong> — Our team will review your message carefully.
                    </td>
                  </tr>
                </table>
              </td>
              ...
              <td style="padding:10px 0;">
                <table cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="width:32px;font-size:18px;vertical-align:top;padding-top:2px;">💬</td>
                    <td style="font-size:14px;color:#2d3d2d;line-height:1.6;">
                      <strong style="color:#1a3d1a;">We'll Reach Out</strong> — Expect a reply at <strong>${data.email}</strong> within 24–48 hours.
                    </td>
                  </tr>
                </table>
              </td>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
      <tr>
        <td style="background:#fff8e1;border-left:4px solid #f59e0b;border-radius:4px;padding:12px 16px;">
          <p style="margin:0;font-size:13px;color:#92400e;">
            ℹ️ This is an automated confirmation. Please <strong>do not reply</strong> to this email — we will reach out to you directly.
          </p>
        </td>
      </tr>
    </table>
  ...
  await sendMail({
    to: data.email,
    subject: "We've Received Your Message – Avshdh Organics",
    html,
  });
}
// ─────────────────────────────────────────────────────────────────
// Contact Admin Notification Email  → sent to the ADMIN
// Usage: sendContactAdminNotificationEmail(adminEmail, contactData)
// ─────────────────────────────────────────────────────────────────
export async function sendContactAdminNotificationEmail(
  ...
  const html = emailWrapper(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:#4a5c4a;line-height:1.6;">
      A visitor has submitted a contact form on <strong style="color:#2d6b2d;">Avshdh Organics</strong>.
      Here are the full details:
    </p>

    <!-- Inquiry Details Card -->
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #d4e8d4;border-radius:10px;overflow:hidden;margin:0 0 28px;">

      <!-- Card Header -->
      <tr>
        <td colspan="2" style="background:linear-gradient(135deg,#1a5c1a,#2d8a2d);padding:14px 20px;">
          <p style="margin:0;font-size:13px;letter-spacing:1.5px;color:#c8f0c8;text-transform:uppercase;font-weight:700;">
      ...
      <tr style="background:#f9fdf9;">
        <td style="padding:14px 20px;font-size:13px;color:#6b7c6b;font-weight:600;width:130px;border-bottom:1px solid #e8f0e8;">Full Name</td>
        <td style="padding:14px 20px;font-size:14px;color:#1a3d1a;font-weight:700;border-bottom:1px solid #e8f0e8;">${data.fullName}</td>
      </tr>
      <tr>
        <td style="padding:14px 20px;font-size:13px;color:#6b7c6b;font-weight:600;border-bottom:1px solid #e8f0e8;">Email</td>
        <td style="padding:14px 20px;font-size:14px;border-bottom:1px solid #e8f0e8;">
          <a href="mailto:${data.email}" style="color:#2d6b2d;font-weight:700;text-decoration:none;">${data.email}</a>
        </td>
      </tr>
      <tr style="background:#f9fdf9;">
        <td style="padding:14px 20px;font-size:13px;color:#6b7c6b;font-weight:600;border-bottom:1px solid #e8f0e8;">Phone</td>
        <td style="padding:14px 20px;font-size:14px;border-bottom:1px solid #e8f0e8;">
          <a href="tel:${data.phone}" style="color:#2d6b2d;font-weight:700;text-decoration:none;">${data.phone}</a>
        </td>
      </tr>
      <tr>
        <td style="padding:14px 20px;font-size:13px;color:#6b7c6b;font-weight:600;border-bottom:1px solid #e8f0e8;">Submitted At</td>
        <td style="padding:14px 20px;font-size:14px;color:#1a3d1a;font-weight:600;border-bottom:1px solid #e8f0e8;">${data.submittedAt}</td>
      </tr>

      <!-- Message Row spans full width -->
      <tr style="background:#f9fdf9;">
        <td colspan="2" style="padding:14px 20px;">
          <p style="margin:0 0 8px;font-size:13px;color:#6b7c6b;font-weight:600;">Message</p>
          <div style="background:#ffffff;border:1px solid #d4e8d4;border-left:4px solid #2d8a2d;border-radius:0 6px 6px 0;padding:14px 16px;">
            <p style="margin:0;font-size:14px;color:#2d3d2d;line-height:1.8;">${data.message}</p>
          </div>
        </td>
      </tr>
    </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
      <tr>
        <td align="center">
          <a href="mailto:${data.email}?subject=Re: Your Inquiry – Avshdh Organics" style="
            display:inline-block;
            background:linear-gradient(135deg,#1a5c1a,#2d8a2d);
            color:#ffffff;
            padding:13px 32px;
    ...
    <p style="margin:0;font-size:13px;color:#8a9e8a;line-height:1.6;">
      This is an automated notification from the Avshdh Organics website contact form.
    </p>
  `);

  await sendMail({
    to: adminEmail,
    subject: `📬 New Contact Inquiry from ${data.fullName} – Avshdh Organics`,
    html,
  });
}
